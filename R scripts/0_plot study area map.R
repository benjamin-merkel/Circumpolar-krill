...
background_polygon <- st_difference((grat.50S),st_geometry(ice_shelf))
background_polygon <- st_difference(background_polygon,st_geometry(land))
background_raster  <- raster(as_Spatial(st_zm(background_polygon)),res=1)
values(background_raster) <- 1:ncell(background_raster)


# env  <- readRDS("data/Circumpolar environmental covariate raster stack updated Dec21.rds")
# bath <- env$bath
# bath[is.na(bath)] <- 0
...
bath  <- project(rast(ibsco), rast(background_raster))
bath[bath>0] <- 0
bath <- raster(bath)
...
png("figures/study area.png", res = 800, width=20, height = 20, units="cm")
par(mar=rep(0,4))
...
arctext(x = "INDIAN OCEAN", center = c(0, 0), radius = 4500, middle = 0*pi/2)
arctext(x = "ATLANTIC OCEAN", center = c(0, 0), radius = 4500, middle = 1.2*pi/2)
arctext(x = "PACIFIC OCEAN", center = c(0, 0), radius = 4500, middle = -1.5*pi/2)
...
text(-3000,  100, "Bellinghausen", cex=sea.cex, col=sea.col)
text(-3000, -100, "Sea", cex=sea.cex, col=sea.col)
